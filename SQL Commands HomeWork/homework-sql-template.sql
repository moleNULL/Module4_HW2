...
WHERE YEAR(ModifiedDate) = 2008 AND MiddleName IS NOT NULL
AND Title IS NULL
...
-------------------------------------------------------------
SELECT DISTINCT Name FROM HumanResources.Department dep
...
-------------------------------------------------------------
SELECT TerritoryID, SUM(CommissionPct) as Sum FROM Sales.SalesPerson 
...
LEFT JOIN Purchasing.PurchaseOrderHeader p ON p.EmployeeID = h.BusinessEntityID
